...
from app.utils import utils, file_security, safety_filters, metadata_gen
...
def generate_audio(task_id, params, video_script):
    ...
    if not custom_audio_file:
        ...
        subject = getattr(params, "video_subject", "Video") or "Video"
    ...
    if params.video_source == "local":
        ...
        downloaded_videos = material.download_videos(
            ...
            video_concat_mode=(
                ...
                if getattr(params, "match_materials_to_script", False)
    ...
    for i in range(params.video_count):
        ...
        if os.path.exists(combined_video_path) and os.path.getsize(combined_video_path) > 0:
            ...
            video.combine_videos(
                ...
                pacing_mode=getattr(params, "pacing_mode", "dynamic"),
                transition_speed=getattr(params, "transition_speed", 0.5),
                apply_ken_burns=getattr(params, "apply_ken_burns", True),
                color_enhancement=getattr(params, "color_enhancement", True),
                enable_pattern_interrupts=getattr(params, "enable_pattern_interrupts", True),
            )
        ...
        if getattr(params, "thumbnail_count", 0) > 0:
            try:
        ...
        if getattr(params, "export_platforms", None):
             try:
        ...
        if getattr(params, "extract_highlights", False):
             try:
        ...
        try:
             analytics_db.log_generation_context(task_id, params, getattr(params, "video_script", ""))
        except Exception as e:
...
def _run_pipeline(task_id, params: VideoParams, stop_at: str = "video"):
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # PHASE 2: SEQUENTIAL EXECUTION — Audio TTS + Material Download
    # ─────────────────────────────────────────────────────────────────────────
