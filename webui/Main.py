...
with left_panel:
    with st.container(border=True):
        ...
        if st.button("🍀 I'm Feeling Lucky", key="feeling_lucky", use_container_width=True):
            with st.spinner("🔮 Consultating the AI Oracle for a viral topic..."):
                try:
                    # Randomly pick a category hint sometimes, or let it be purely random
                    categories = ["Mystery", "Facts", "Psychology", "Finance", "Health", "History", "Science", "Horror", "Motivation"]
                    cat_hint = random.choice(categories) if random.random() > 0.3 else ""
                    
                    lucky_topic = llm.generate_viral_topic(cat_hint)
